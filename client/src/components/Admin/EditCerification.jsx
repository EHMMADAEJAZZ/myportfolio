...
const EditCerification = ({data,onClose}) => {
     const [certificatData, setCertificateData] = useState({
      ...
      const [previewImg, setPreviewImg] = useState(data?.certificationImage);
      ...
      const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
          const response = await certificationApis.updateCertifications(data?._id,formData)
  ...
  return (
     <Model onClose={onClose} className='bg-black/70' title='add Certifaction'>
      <form
...
export default EditCerification